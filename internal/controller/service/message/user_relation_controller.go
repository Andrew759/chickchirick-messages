...
func (urc *UserRelationController) GetUserRelations(c *gin.Context) {
	...
	userRelations, err := userRelation.GetUserRelation(ctx, urc.Controller.DI.DBDecorator.GDB())
	if err != nil {
...
func (urc *UserRelationController) GetUserRelation(c *gin.Context) {
	...
	ctx := c.Request.Context()
	ur, err := userRelation.GetUserRelationById(ctx, urc.Controller.DI.DBDecorator.GDB(), id)
	if err != nil {
...
func (urc *UserRelationController) CreateUserRelation(c *gin.Context) {
	...
	ctx := c.Request.Context()
	if err := userRelation.CreateUserRelation(ctx, urc.Controller.DI.DBDecorator.GDB(), &ur); err != nil {
...
func (urc *UserRelationController) UpdateUserRelation(c *gin.Context) {
	...
	ctx := c.Request.Context()
	err = userRelation.UpdateUserRelationById(ctx, urc.Controller.DI.DBDecorator.GDB(), &ur, id)
	if err != nil {
...
func (urc *UserRelationController) DeleteUserRelation(c *gin.Context) {
	...
	ctx := c.Request.Context()
	err = userRelation.DeleteUserRelationById(ctx, urc.Controller.DI.DBDecorator.GDB(), id)
	if err != nil {
		if errors.Is(err, userRelation.UserRelationNotFoundErr) {
			c.JSON(http.StatusNotFound, gin.H{"error": err.Error()})
		} else {
